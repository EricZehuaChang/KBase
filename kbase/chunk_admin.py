...
# 与 ingest/pipeline.py、reindex.py 完全一致的向量化文本组成——三处必须
# 同步演进，否则编辑后的向量与摄取向量的语义空间不一致。
def _embed_text(c: Chunk) -> str:
    return f"{c.enrich_context or ''}\n{c.heading_path}\n{c.text}".lstrip()


def _kw_text(c: Chunk) -> str:
