...
export function createShareLink(
  kbId: string, body: { name?: string; provider?: string | null },
...
/** 分享页首屏（免登录）：库名 + 链接备注。404=链接不存在或已撤销。 */
export function getShareMeta(token: string): Promise<{ kb_name: string; name: string }> {
