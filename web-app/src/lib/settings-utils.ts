...
/** 厂商标识（Provider 卡片 logo）。按 base_url 域名识别厂商：已知厂商用
 * 官方矢量 logo（assets/vendors/ 内置的 lobe-icons SVG，随包打包离线可用，
 * 渲染在 VendorLogo.vue）；未知域名（企业自有 OpenAI 兼容平台）回落
 * "品牌色圆角块 + 缩写"中性徽章。纯函数，vitest 单测。 */
export interface VendorBadge {
  /** 厂商全名（tooltip/无障碍标签） */ label: string;
  /** 官方 logo 资产名（assets/vendors/ 文件；无=用缩写徽章回落） */ icon?: string;
  /** 回落徽章缩写（1~4 字符） */ short: string;
  /** 回落徽章底色 */ color: string;
}

const VENDORS: [RegExp, VendorBadge][] = [
  [/bigmodel\.cn/i, { label: "智谱 GLM", icon: "zhipu", short: "GLM", color: "#3859FF" }],
  [/dashscope|aliyuncs\.com/i, { label: "阿里云通义", icon: "qwen", short: "QW", color: "#615CED" }],
  [/deepseek\.com/i, { label: "DeepSeek", icon: "deepseek", short: "DS", color: "#4D6BFE" }],
  [/openai\.com/i, { label: "OpenAI", icon: "openai", short: "GPT", color: "#10A37F" }],
  [/moonshot\.cn/i, { label: "Moonshot Kimi", icon: "kimi", short: "K", color: "#16191E" }],
  [/siliconflow\.cn/i, { label: "硅基流动", icon: "siliconcloud", short: "SF", color: "#7C3AED" }],
];
